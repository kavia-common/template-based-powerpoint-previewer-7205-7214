...
function SlideHtml({ slide, mode }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: hasImage && slide.layout.includes("image") ? "1fr 0.95fr" : "1fr", gap: 16, marginTop: isFull ? 18 : 10 }}>
          ...
          {Array.isArray(slide.bullets) && slide.bullets.some((b) => String(b || "").trim()) ? (
            <ul style={{ margin: 0, paddingLeft: 18, marginTop: slide.summary ? 10 : 0 }}>
              {slide.bullets
                ...
                .slice(0, isFull ? 12 : 6)
                .map((b, idx) => (
                  <li key={idx} style={{ fontSize: bodySize, color: slide.theme.colors.text, opacity: 0.92, marginBottom: 6 }}>
                    {b}
                  </li>
                ))}
            </ul>
